...
def main():
    ...
    for source_name in SOURCE_NAMES:
        ...
        data_duration_dict[source_name] = 0
        ...
        for file_path in pbar:
            ...
            try:
                ...
                for item in tqdm(data_list, desc=f"Processing {file_path}", leave=False):
                    ...
                    data_duration_dict[source_name] += processed_data.duration

            except Exception as e:
                print(f"\n处理文件 {file_path} 时出错: {traceback.format_exc()}")
            
            print(f"{file_path} 处理完成，跳过 cer: {cer_skip_num}, not_silence_skip_num: {not_silence_skip_num}, diarization: {diarization_skip_num}")

    print(data_num_dict)
    for source_name, duration in data_duration_dict.items():
        print(f"{source_name} 总时长: {(duration / 3600):.2f} h")
    print(f"总时长: {sum(data_duration_dict.values()) / 3600:.2f} h")
