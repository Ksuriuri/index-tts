...
class AudioLoaderWorker(Process):
    def __init__(self, file_queue: Queue, gpu_task_queue: Queue, worker_id: int, checkpoint: Dict[str, int]):
        ...
        while True:
            ...
            try:
                ...
                for batch_data in parquet_file.iter_batches(batch_size=256, columns=['audio', 'text']):
                    ...
                    for i in range(len(batch_data)):
                        ...
                        try:
                            ...
                            task = ASRTask(
                                ...
                                audio_raw=array.astype(np.float32), 
                                sample_rate=sr, 
                                text_gt=text,
                    ...
                    rows_read += batch_len
                logger.info(f"[Loader-{self.worker_id}] Finished loading {parquet_path}")
...
class GPUASRWorker(Process):
    ...
    def run(self):
        ...
        while True:
            ...
            for task in tasks:
                try:
                    audio_tensor = torch.from_numpy(task.audio_raw).to(device_str)
...
class ParquetWriterWorker(Process):
    ...
    def __init__(self, result_queue: Queue, output_dir: str, save_interval: int, checkpoint_path: str, initial_checkpoint: dict):
        ...
        self.checkpoint = initial_checkpoint # 这里的 checkpoint 是总表

    def run(self):
        os.makedirs(self.output_dir, exist_ok=True)
        buffer = []
        
        # 统计本轮保存周期内，每个源文件贡献了多少个样本
        ...
        pbar = tqdm(desc="Processed Samples")
    ...
    def _save(self, data_list, idx, cycle_counts):
        ...
        logger.info(f"Saved {len(data_list)} samples to {save_path}. Checkpoint updated.")
...
def main():
    ...
    all_parquet_files = glob.glob(os.path.join(DATASET_ROOT, "**/*.parquet"), recursive=True)
    
    file_queue = Queue()
    for f in all_parquet_files:
        file_queue.put(f)
    
    gpu_task_queue = Queue(maxsize=DEVICE_NUM * 8) 
    result_queue = Queue()
    ...
    for i in range(CPU_WORKERS_NUM):
        p = AudioLoaderWorker(file_queue, gpu_task_queue, i, checkpoint)
        p.start()
